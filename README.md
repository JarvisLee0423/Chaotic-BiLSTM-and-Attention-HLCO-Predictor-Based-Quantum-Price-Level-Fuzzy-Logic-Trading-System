...
- We introduce a novel fuzzy logic-based trading strategy combined with the above price predictor to remedy fixed order-triggering boundaries and the dilemma of traditional finance indicators. 
...
The architecture of Chaotic HLCO Predictor, which employs the Bi-LSTM to extract time-series features from the previous price data and replaces the activation function with Chaotic Neuro-Oscillators to simulate the high randomness in the market fluctuations.
...
The ablation study of the different types of Chaotic Neuro-Oscillator in the accuracy of the price prediction.
...
The backtest trading results of 10 Futures (CFL-X is our Chaotic Fuzzy Logic Trading Strategy with type-x Chaotic Neuro-Oscillator in the HLCO Price Predictor).
...
**Step 2**: Run the code to get the training data.
...
**Hint**: All the hyperparameter configurations are placed in the Params.txt file.
...
- `LeeSigType` is the type of the Lee-Oscillator-based sigmoid activation function.
- `K` is the hyper-parameter in the Lee-Oscillator.
- `N` is the hyper-parameter in the Lee-Oscillator.
- `Chaotic` is the controller of whether to use the Lee-Oscillator to form the chaotic activation function.
...
- `learningRate` is the learning rate for gradient descent.
...
- `epoches` is the number of the training epochs.
...
1. `Prediction`
    ...
    python Predictor.py
    ```
    **Hint**: Pay attention to the directories you configured in the Params.txt. All the CSV files should be put correctly.
...
## Acknowledgements
This paper was supported by Research Grant R202008 of Beijing Normal University-Hong Kong Baptist University United International College (UIC) and Key Laboratory for Artificial Intelligence and Multi-Model Data Processing of the Department of Education of Guangdong Province.
